    ...
    public static void main(String[] args)
    {
        ...
        try 
        {
          ...
          //  IMatrix sum1 = matrix1.sum(matrix2);
            ...
            IMatrix sum2 = matrix2.sum(matrix1);

            long endLLSum = System.currentTimeMillis();

            System.out.println("time elapsed for sum m2 (sparse) + m1 (usual)" + (endLLSum - startLLSum));

            long startHMSum = System.currentTimeMillis();

            IMatrix sumHM = matrix4.sum(matrix1);
            
            long endHMSum = System.currentTimeMillis();

            System.out.println("time elapsed for sum  m3 (sparseHM) + m1 (usual)" + (endHMSum - startHMSum));
           ...
           // IMatrix product1 = matrix1.product(matrix2);
            ...
            IMatrix product2 = matrix2.product(matrix1);

            long endLL = System.currentTimeMillis();

            System.out.println("time elapsed for multiplication m2 (sparse) * m1 (usual)" + (endLL - startLL));

            long startHM = System.currentTimeMillis();

            IMatrix productHash = matrix4.product(matrix1);

            long endHM = System.currentTimeMillis();

            System.out.println("time elapsed for multiplication m3 (sparseHM) * m1 (usual)" + (endHM - startHM));
